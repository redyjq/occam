...
int main(int argc, char** argv) {
    ...
    OccamImage* combinedImage = captureImage(device, OCCAM_STITCHED_IMAGE1);

    printf("Combined image width: %d\n", combinedImage->width);
    printf("Combined image height: %d\n", combinedImage->height);

    for (int h = 480; h < combinedImage->height; ++h) {
    	for (int w = 0; w < combinedImage->width; ++w) {
            int index = (h * combinedImage->height) + w;
            printf("R: %d, G: %d, B: %d\n", combinedImage->data[0][index*3], combinedImage->data[0][index*3 + 1], combinedImage->data[0][index*3 + 2]);
    	}
    }
    /*
